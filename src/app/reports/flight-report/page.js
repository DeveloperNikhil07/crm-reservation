...
export default function FlightReports() {
    ...
    const handleForm = (e) => {
        ...
        if (!ReportFormData.name) validationErrors.name = 'Name is required';
        if (!ReportFormData.email) validationErrors.email = 'Email is required';
        else if (!/\S+@\S+\.\S+/.test(ReportFormData.email)) validationErrors.email = 'Invalid email';
    ...
    useEffect(() => {
        ...
        const status = queryParams.get('status') || "All"; // Default to "All" if no status
    ...
    const ReportDataStatus = async (dataRequestBody, setReportData) => {
        try {
            const ReportStatusRes = await FlightReportApi(dataRequestBody);
            const ReportResData = ReportStatusRes.data;

            setReportData(ReportResData?.flight_Booking_Report_Details || []);
    ...
    const fetchStatusData = async () => {
        ...
        const dataRequestBody = {
            bookingStatus: confirmStatus === "All" ? null : confirmStatus, // Handle "All"
        ...
        try {
            ...
            setReportResponseData(ReportResData?.flight_Booking_Report_Details || []);
        } catch (err) {
                                ...
                                <form action="" method='post' onSubmit={handleForm} className="row">
                                    ...
                                    <div className="col-12 col-md-3 mt-3">
                                        <div className='form-fields'>
                                            ...
                                            <select name="bookingStatus" value={ReportFormData.bookingStatus || confirmStatus || ""} onChange={handleInput} id="booking_status" className='form-control'>
                                        ...
                                        <>
                                            ...
                                            <div className="col-12 col-md-3 mt-3">
                                                <div className='form-fields'>
                                                    ...
                                                    <input type="text" name='bookingAmount' value={ReportFormData.bookingAmount || ""} onChange={handleInput} className='form-control' id='booking_amount' placeholder='Enter Amount' />
                                                </div>
                                            </div>
                                            <div className="col-12 col-md-3 mt-3">
                                                <div className='form-fields'>
